...
- [ ] Explain benefit(s) using `client-side routing`?
  > Answer: Javascript within a client app manages the data for that app in it's own memory. This means that the browser no longer needs to refresh between routes.
- [ ] What does AJAX stand for?
  > Answer: Asynchronous JavaScript and XML
- [ ] What are `controlled components` in React?
  > Answer:Constrolled components used so that the data on the DOM is controlled by the programmer with state rather than form inputs and other DOM elements.
